...
const guest = {
  ...
  const suitableRecipes = recipes.filter((recipe)=>{
    ...
    const favIngredient = recipe.ingredients.some((ingredient)=>{
       return guest.loves.includes(ingredient)
    });

    const dislikedIngredients=recipe.ingredients.some((ingredient)=>{
        return guest.dislikes.includes(ingredient)
    });

    return favIngredient && !dislikedIngredients;
